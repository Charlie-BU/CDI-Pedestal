...
import { getApiBase, saveDebugSettings } from "@/localDebug";
...
describe("CDI debug transport", () => {
    it.each(["", "http://localhost:9101"])("keeps shell requests on /api/cam with remote override %s and preserves auth and cancellation", async (backend) => {
        ...
        expect(requests[0].baseURL).toBe("/api/cam");
        expect(getApiBase("cam")).toBe(backend || "/api/cam");
